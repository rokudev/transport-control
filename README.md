...
This sample includes both standard and custom video player channels: 
...
The **ExtVideoPlayerUI** component in the custom UI channel takes the transport event and passes it to a method that processes it based on its type. The channel explicitly handles "seek" and "start over" commands, which is required for implementing custom trick mode, while letting the Roku OS implicitly handle "fast forward", "rewind", and "replay" commands. The channel then displays the updated position on the progress bar, and the control button corresponding to the event being executed.
...
2. In the extracted **transport-control-master** folder, expand the **Transport_Control_NativeUI** folder to access the standard UI example, or expand the **Transport_Control_CustomUI** folder for the custom UI example. Compress the contents in the expanded folder to a ZIP file.
